...
interface GuzelYazmaActivityProps {
  onBack: () => void;
  students: Student[];
  user: any;
  onShowInfo?: () => void;
...
const CRITERIA = {
  'Metin': [
    { id: 'harf_formu', label: 'Harflerin Doğru Formu', max: 10 },
    { id: 'bosluklar', label: 'Kelime ve Satır Arası Boşluklar', max: 10 },
    { id: 'sayfa_duzeni', label: 'Sayfa Düzeni ve Kenar Boşlukları', max: 10 },
    { id: 'yazim_noktalama', label: 'Yazım ve Noktalama Kuralları', max: 10 },
    { id: 'estetik', label: 'Genel Estetik ve Okunabilirlik', max: 10 }
  ],
  'Şiir': [
    { id: 'misra_duzeni', label: 'Mısra Düzeni ve Hizalama', max: 10 },
    { id: 'estetik_harf', label: 'Harflerin Estetik Görünümü', max: 10 },
    { id: 'temizlik', label: 'Sayfa Düzeni ve Temizlik', max: 10 },
    { id: 'yazim_noktalama', label: 'Yazım ve Noktalama Kuralları', max: 10 },
    { id: 'form_uygunluk', label: 'Başlık ve Şiir Formuna Uygunluk', max: 10 }
  ]
};
...
export const GuzelYazmaActivity: React.FC<GuzelYazmaActivityProps> = ({ onBack, students, user, onShowInfo }) => {
  ...
  const cumulativeScores = useMemo(() => {
    ...
    allScores.filter(s => instances.some(i => i.id === s.instanceId)).forEach(score => {
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-3xl font-black text-neutral-900 dark:text-white tracking-tight">
                Güzel Yazma Etkinliği
              </h2>
              <button 
                onClick={onShowInfo}
                className="p-1.5 text-neutral-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg transition-all"
            ...
            <p className="text-neutral-500 dark:text-neutral-400 font-medium">
              Yazı yarışmalarını yönetin, estetik ve kurallara göre değerlendirin.
            </p>
          </div>
        </div>
          ...
          >
            <div className="flex items-start justify-between">
              <div className={`p-3 ${instance.type === 'Metin' ? 'bg-sky-50 text-sky-600' : 'bg-rose-50 text-rose-600'} rounded-2xl`}>
                {instance.type === 'Metin' ? <FileText size={24} /> : <PenTool size={24} />}
              </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Örn: 1. Güzel Yazma Yarışması"
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-neutral-500 text-sm font-medium">Tüm Güzel Yazma etkinliklerinin toplam puanları.</p>
                    </div>
                  </div>
...
const ResultEntryModal: React.FC<ResultEntryModalProps> = ({ isOpen, onClose, instance, students, user, allScores }) => {
      ...
      for (let i = 0; i < rankedScores.length; i++) {
        ...
        await updateDoc(studentRef, {
          ...
          starHistory: arrayUnion({
            ...
            description: `Güzel Yazma - ${instance.name} Etkinliği Ödülü`,
            amount: reward,
            timestamp: Date.now()
          })
        });
  ...
  const confirmDelete = async () => {
    ...
    try {
      // Delete scores
      const q = query(collection(db, `users/${user.uid}/activityScores`), where('instanceId', '==', instance.id));
      const snap = await getDocs(q);
      for (const d of snap.docs) {
        await deleteDoc(d.ref);
      }
      // Delete instance
      await deleteDoc(doc(db, `users/${user.uid}/activityInstances`, instance.id));
          ...
          >
            ...
            <div className="flex-1 overflow-hidden flex">
              {viewMode === 'entry' ? (
                      ...
                      <div className="space-y-8">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-neutral-500 font-medium">Yazı Değerlendirme Puanlarını Girin</p>
                          </div>
                          <div className="text-right">
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                  <div className="flex items-center justify-between mb-8">
                    <h4 className="text-2xl font-black text-neutral-900 dark:text-white uppercase tracking-tight">Yarışma Sonuçları</h4>
                    <button 
                      onClick={() => {
                        const doc = new jsPDF();
                        doc.setFont("helvetica", "bold");
                        doc.text(fixTurkishChars(`${instance.name} - Sonuclari`), 105, 15, { align: 'center' });
                        
                        const headers = [[
                          fixTurkishChars('Sira'), 
                          fixTurkishChars('Adi Soyadi'), 
                          fixTurkishChars('Toplam Puan'),
                          fixTurkishChars('Odul')
                        ]];
                        
                        const ranked = (Object.values(localScores) as ActivityScore[]).sort((a, b) => b.totalScore - a.totalScore);
                        const data = ranked.map((s, idx) => [
                          idx + 1,
                          fixTurkishChars(s.studentName),
                          s.totalScore,
                          s.rewardStars ? `${s.rewardStars} Yildiz` : '-'
                        ]);
                        ...
                        doc.save(`${fixTurkishChars(instance.name)}_sonuclari.pdf`);
                      }}
                      className="flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-bold transition-all shadow-lg shadow-emerald-100 dark:shadow-none"
                    >
                      <Download size={20} />
                      Sonuçları İndir
                    </button>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {(Object.values(localScores) as ActivityScore[]).sort((a, b) => b.totalScore - a.totalScore).map((score, idx) => (
                      <div 
                        key={score.studentId}
                        className={`flex items-center justify-between p-6 rounded-3xl border ${
                          idx < 3 ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/10 dark:border-amber-900/30' : 'bg-white border-neutral-100 dark:bg-neutral-800 dark:border-neutral-700'
                        }`}
                      >
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <p className="text-lg font-black text-neutral-900 dark:text-white uppercase tracking-tight">{score.studentName}</p>
                            <div className="flex gap-2 mt-1">
                              {score.rewardStars && (
                                <span className="flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-[10px] font-black uppercase tracking-widest">
                                  <Star size={10} fill="currentColor" />
                                  {score.rewardStars} Yıldız Ödülü
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="text-right">
                          <p className="text-3xl font-black text-neutral-900 dark:text-white">{score.totalScore}</p>
                          <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">Toplam Puan</p>
                        </div>
                      </div>
                    ))}
